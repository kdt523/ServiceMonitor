# ServiceMonitor — Deployment Guide (Render + Vercel)
...
User Browser
  ...
  Vercel (CDN / Proxy)          ← your-app.vercel.app
     ...
     ▼
  Render (FastAPI App)          ← servicemonitor-backend.onrender.com
...
5. It will create:
   - A **Web Service** (`servicemonitor-backend`) running the Docker container
   - A **PostgreSQL database** (`servicemonitor-db`) — `DATABASE_URL` is auto-injected
...
```
https://servicemonitor-backend.onrender.com
```
...
Edit `servicemonitor/frontend/vercel.json` and replace the URL with your actual Render service URL:
...
Commit and push:
```bash
git add servicemonitor/frontend/vercel.json
...
3. In **Root Directory**, type: `servicemonitor/frontend`

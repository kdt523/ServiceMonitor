/**
 * app.js — Global WebSocket manager + monitoring toggle for Gonitor.
 *
...
(function () {
  ...
  function connect() {
    ...
    ws.addEventListener('open', () => {
      console.debug('[Gonitor WS] Connected');
      clearTimeout(reconnectTimer);
      // Notify any page-level handlers that the WS is ready
      if (window._gonitorReady) {
        window._gonitorReady.forEach(fn => fn(ws));
      }
    });

    ws.addEventListener('close', () => {
      console.debug('[Gonitor WS] Disconnected — reconnecting in', RECONNECT_MS, 'ms');
      reconnectTimer = setTimeout(connect, RECONNECT_MS);
    });

    ws.addEventListener('error', err => {
      console.warn('[Gonitor WS] Error:', err);
    });
  ...
  function handleGlobalEvent(msg) {
    ...
    if (event === 'app-starting') {
      console.info('[Gonitor] App starting');
    }
  }
  ...
  window.gonitorShowToast = showToast;
})();

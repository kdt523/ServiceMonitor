...
async def save_settings(
    ...
    s.site_name = site_name or "Gonitor"
...
async def test_email(
    ...
    try:
        ok = await send_email(
            ...
            subject="Gonitor — Test Email",
            body_html="<p>This is a test email from <strong>Gonitor</strong>.</p>",
            body_text="This is a test email from Gonitor.",
            raise_on_error=True,
        )
...
async def test_sms(
    ...
    try:
        ok = await send_sms(
            ...
            body="Gonitor test SMS — your alerts are working!",
            raise_on_error=True,
        )
        return JSONResponse({"success": ok})
    except Exception as exc:

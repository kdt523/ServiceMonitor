/**
 * dashboard.js — ServiceMonitor real-time dashboard
 *
 * Responsibilities:
...
(async function initPusher() {
  ...
  if (!pusherKey) {
    console.warn('[ServiceMonitor] Pusher key not configured — real-time updates disabled.');
    return;
  }
  ...
  const globalChannel = pusher.subscribe('servicemonitor-global');
...
function showToast(data) {
  ...
  el.id = toastId;
  el.className = `alert alert-${color} alert-dismissible fade show servicemonitor-toast`;
  el.role = 'alert';
  el.innerHTML = `
...
document.querySelectorAll('.check-now-btn').forEach((btn) => {
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[ServiceMonitor] check-now error:', e);
    } finally {
      btn.disabled = false;
...
if (confirmDeleteBtn) {
  confirmDeleteBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[ServiceMonitor] delete error:', e);
    } finally {
